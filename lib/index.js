...
class PlayQueue {
  ...
  setOpts(opts) {
    const settableOpts = [
      ...
      {'key': 'limit'},
      {'key': 'localStorageNS'},
      {'key': 'shouldNotifyBeforeEnd', 'obj': 'audioManager'},
      {'key': 'progressEvents', 'obj': 'audioManager'},
      {'key': 'minuteEvents', 'obj': 'audioManager'},
      {'key': 'heartbeat', 'obj': 'audioManager'}
    ];
    settableOpts.forEach(settableOpt => {
      if (opts[settableOpt.key] !== undefined) {
        if (settableOpt.obj) {
          this[settableOpt.obj][settableOpt.key] = opts[settableOpt.key];
        } else {
          this[settableOpt.key] = opts[settableOpt.key];
        }
      }
    });
