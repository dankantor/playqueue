...
class AudioManager {
  ...
  timeUpdate() {
    ...
    if (this.progressEvents === true) {
      let progressPercentage = Math.floor((this.audio.currentTime / this.audio.duration) * 100);
    ...
    if (this.heartbeat > 0) {
      let heartbeatRemainder = Math.floor(this.audio.currentTime % this.heartbeat);
